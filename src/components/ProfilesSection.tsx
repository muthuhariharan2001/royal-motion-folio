        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {profiles.map((profile, index) => (
            <DraggableCard
              key={profile.name}
              id={`profile-${index}`}
              className="premium-card hover:scale-105 transition-all duration-300"
            >
              <a
                href={profile.url}
                target="_blank"
              ...
              >
                <div className={`w-16 h-16 mx-auto mb-4 rounded-full ${profile.color} flex items-center justify-center transition-colors duration-300`}>
                  {profile.icon ? (
